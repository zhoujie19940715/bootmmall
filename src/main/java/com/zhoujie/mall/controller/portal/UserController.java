...
public class UserController {
    ...
    public ServerResponse<User> login(String username, String password, HttpSession session, HttpServletResponse httpServletResponse) {
        ...
        if (response.isSuccess()) {
            //说明：面试必考！！！
            //// TODO: 2019-04-23  ①将在服务端生成的cookie对象响应给浏览器保存
            CookieUtil.writeLoginToken(httpServletResponse,session.getId());
            //// TODO: 2019-04-23  ②用seesion的id作为key,user对象序列化的json字符串作为值，过期时间为30分钟，保存到redis数据库
    ...
    public ServerResponse<String> logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        //// TODO: 2019-04-23   ①删除缓存
        RedisShardedPoolUtil.del(loginToken);
        //// TODO: 2019-04-23   ②删除cookie
    ...
    public ServerResponse<User> getUserInfo(HttpSession session, HttpServletRequest request) {
       ...
       //// TODO: 2019-04-23 ①读取cookie
        String loginToken = CookieUtil.readLoginToken(request);
        if (StringUtils.isEmpty(loginToken)){
            return ServerResponse.createByErrorMessage("用户未登录,无法获取当前用户的信息");
        }
        //// TODO: 2019-04-23  ②从redis中获取user的序列
        String userJson = RedisShardedPoolUtil.get(loginToken);
        //// TODO: 2019-04-23  ③反序列化,生成user对象
